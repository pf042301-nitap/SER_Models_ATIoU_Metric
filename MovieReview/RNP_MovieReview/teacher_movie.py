#python -u teacher_beer.py --save 1 --dropout 0.2 --lr 0.0001 --batch_size 128 --gpu 0 --sparsity_percentage 0.167 --sparsity_lambda 11 --continuity_lambda 12 --epochs 200 --aspect 0
import argparse
import os
import time
...
# collect all numerical parameters except selected ones
exclude_params = {
    "dropout",
    "embedding_dim",
    "hidden_dim",
...
# create model name
RNP_model_name = "RNP_model_" + "_".join(
    [f"{k}_{v}" for k, v in sorted(numeric_params.items())]
) + ".pth"

print(RNP_model_name)
...
# Safe GPU device handling with automatic detection
if torch.cuda.is_available():
    available_gpus = torch.cuda.device_count()
    print(f"Available GPUs: {available_gpus}")
    
    # Use GPU 0 if only one GPU is available, otherwise use the specified GPU
    if available_gpus == 1:
        device_id = 0
        print(f"Only 1 GPU available. Using GPU: {device_id}")
    else:
        device_id = min(int(args.gpu), available_gpus - 1)
...
train_data = MovieData(args.data_dir, 'train', word2idx, balance=True)
...
# ==============================================================
# ==============================================================
# Results filename using SAME pattern as model filename
# ==============================================================
results_filename = os.path.join(
    './trained_model/',
    RNP_model_name.replace(".pth", ".txt")
)
