...
def parse():
    ...
    parser.add_argument('--data_dir',
                    ...
                    help='Path of the movie dataset')
    ...
    parser.add_argument('--annotation_path',
                    ...
                    help='Path to movie annotations')
    ...
    parser.add_argument('--model_base_dir',
                    type=str,
                    default='./trained_model/movie',
                    help='Directory containing trained movie models')
...
# load dataset
######################
dev_data = MovieData(args.data_dir, 'dev', word2idx)
annotation_data = MovieAnnotation(args.annotation_path, word2idx)
...
# Construct model path matching your actual naming convention
model_params = {
...
for k, v in sorted(model_params.items()):
    param_strs.append(f"{k}_{v}")

RNP_model_name = "RNP_model_" + "_".join(param_strs) + ".pth"
...
print(f"Loading RNP model from: {model_path}")
...
try:
    if os.path.exists(model_path):
        ...
        try:
            ...
            print("RNP model loaded successfully from state_dict")
        except Exception as e:
            print(f"Error loading as state_dict: {e}")
            print("Attempting to load as full model...")
            # Load as full model
            model = torch.load(model_path, map_location=device)
            print("RNP model loaded successfully as full model")
    else:
        # Try alternative path without beer_results prefix
        alt_model_path = f'./trained_model/movie/RNP_model_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}.pkl'
        if os.path.exists(alt_model_path):
            ...
            try:
                ...
                model = torch.load(alt_model_path, map_location=device)
                print("Model loaded successfully as full model from alternative path")
        else:
...
with torch.no_grad():
    for batch_idx, (inputs, masks, labels, annotations) in enumerate(annotation_loader):
        ...
        # Forward pass - RNP model returns rationales and logits
        # Try different forward pass methods
        try:
            # Try the stanRNPd GenEnc model forward pass
            rationales, logits = model(inputs, masks)
            has_rationales = True
        except Exception as e:
            print(f"Error with stanRNPd forward pass: {e}")
        ...
        for i in range(batch_size):
            ...
            if has_rationales and rationales is not None:
                try:
                    ...
                    gen_rationale_text = get_rationale_words(
                        review_tokens,
                        gen_rationale_mask,
                        idx2word,
                        review_mask
                    ...
                    gen_highlighted = highlight_rationale_in_text(
                        review_tokens,
                        gen_rationale_mask,
                        idx2word,
                        review_mask
                    ...
                    print(f"Rationale extraction failed: {e}")

                    gen_rationale_mask = np.zeros_like(
                        annotations[i].cpu().numpy()
                    )
            ...
            for t in thresholds:
                ...
                for r in R:
                    ...
                    # # ===== to =====
                    # if best_iou >= t and best_idx is not None:
                    #     TP_to += 1
                ...
                metrics.update({
                    ...
                    # f"t_{t}_to_tp": TP_to,
                    # f"t_{t}_to_fp": len(R) - TP_to,
                    # f"t_{t}_to_fn": len(A) - len(matched),
...
csv_filename = RNP_model_name.replace('.pth', '_metrics.csv')
...
os.makedirs(output_dir, exist_ok=True)
results_filename = RNP_model_name.replace('.pth', '_summary.txt')
...
with open(results_filepath, 'w') as f:
    ...
    for t in thresholds:
        ...
        # ERASER
        p = np.mean(macro[t]['eraser_prec'])
        r = np.mean(macro[t]['eraser_rec'])
        f1 = np.mean(macro[t]['eraser_f1'])
        f.write(f"  ERASER -> P: {p:.4f}, R: {r:.4f}, F1: {f1:.4f}\n")

        # to
        p = np.mean(macro[t]['to_prec'])
        r = np.mean(macro[t]['to_rec'])
        f1 = np.mean(macro[t]['to_f1'])
        f.write(f"  to -> P: {p:.4f}, R: {r:.4f}, F1: {f1:.4f}\n")

        # Storek
        p = np.mean(macro[t]['storek_prec'])
        r = np.mean(macro[t]['storek_rec'])
        f1 = np.mean(macro[t]['storek_f1'])
        f.write(f"  Storek -> P: {p:.4f}, R: {r:.4f}, F1: {f1:.4f}\n")

        # Modified
        p = np.mean(macro[t]['mod_prec'])
        r = np.mean(macro[t]['mod_rec'])
        f1 = np.mean(macro[t]['mod_f1'])
        f.write(f"  Modified -> P: {p:.4f}, R: {r:.4f}, F1: {f1:.4f}\n")

print(f"\nSummary saved to: {results_filepath}")

# # Print sample output
# print("\nSample of first 3 reviews:")
# for i in range(min(3, len(results))):# Print sample output
#     print("\nSample of first 3 reviews:")
#     for i in range(min(3, len(results))):
#         print(f"\nReview ID: {results[i]['review_id']}")
#         print(f"Review: {results[i]['review'][:150]}...")
#         print(f"Generated rationale (underscore format): {results[i]['generated_rationale'][:150]}...")
#         print(f"Annotated rationale (underscore format): {results[i]['annotated_rationale'][:150]}...")
#         print(f"Generated rationale (highlighted): {results[i]['generated_highlighted'][:150]}...")
#         print(f"Annotated rationale (highlighted): {results[i]['annotated_highlighted'][:150]}...")
#         print(f"Predicted label: {results[i]['pred_label']} (0=negative, 1=positive)")
#         print(f"Ground truth label: {results[i]['gt_label']} (0=negative, 1=positive)")
#         print("-" * 80)
#         print(f"\nReview ID: {results[i]['review_id']}")
#         print(f"Review: {results[i]['review'][:150]}...")
#         print(f"Generated rationale (underscore format): {results[i]['generated_rationale'][:150]}...")
