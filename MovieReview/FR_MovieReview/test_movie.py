...
def parse():
    ...
    parser.add_argument('--batch_size',
                        type=int,
                        default=32,  # Testing batch size (different from training)
...
for k, v in sorted(numeric_params.items()):

    # ONLY fix taudecay
    if k == "taudecay" and isinstance(v, float) and v.is_integer():
        v = int(v)

    FR_model_parts.append(f"{k}_{v}")

FR_model_name = "FR_model_" + "_".join(FR_model_parts) + ".pth"

# move this AFTER filename generation
args.tau = [args.tau]
...
# Use testing batch size (different from training batch size)
...
# Determine model path based on training batch size
# if args.batch_size == 256:
#     model_path = './trained_model/256_Batch_Size/beer/FR_model_share_{}_sparsity_{}_lr_{}_conl_{}_spl_{}_aspect_{}.pkl'.format(
#         args.share, args.sparsity_percentage, args.lr, args.continuity_lambda, args.sparsity_lambda, args.aspect
#     )
# elif args.batch_size == 128:
#     model_path = './trained_model/128_Batch_Size/beer/FR_model_share_{}_sparsity_{}_lr_{}_conl_{}_spl_{}_aspect_{}.pkl'.format(
#         args.share, args.sparsity_percentage, args.lr, args.continuity_lambda, args.sparsity_lambda, args.aspect
#     )
...
try:
    if os.path.exists(model_path):
        ...
        # alt_model_path = os.path.join(args.model_base_dir, f'FR_model_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}_aspect_{args.aspect}.pkl')
        ...
        if os.path.exists(alt_model_path):
            ...
            raise FileNotFoundError(f"Model file not found at either location")
...
# if args.train_batch_size == 256:
#     csv_filename = f"Metric_FR_test_results_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}_aspect_{args.aspect}_trainbs{args.batch_size}.csv"
# elif args.train_batch_size == 128:
#     csv_filename = f"Metric_FR_test_results_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}_aspect_{args.aspect}_trainbs{args.batch_size}.csv"
csv_filename = FR_model_name.replace(
    ".pth",
    "_metric_results.csv"
)
...
# os.makedirs('./test_model/test_beer/', exist_ok=True)
os.makedirs('./test_model/test_movie/', exist_ok=True)
# if args.train_batch_size == 256:
#     results_filename = f"Metric_FR_test_summary_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}_aspect_{args.aspect}_trainbs{args.batch_size}.txt"
# elif args.train_batch_size == 128:
#     results_filename = f"Metric_FR_test_summary_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}_aspect_{args.aspect}_trainbs{args.batch_size}.txt"
results_filename = FR_model_name.replace(
                                            ".pth",
                                            "_metric_summary.txt"
                                        )
