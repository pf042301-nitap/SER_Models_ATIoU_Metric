...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="fever")
...
def check_evidences_sent_len(evidence, annotation_id):
    if (evidence["end_sentence"] - evidence["start_sentence"]) > 1:
        print(f"evidence longer than one sentence: {annotation_id}")


def check_evidences_format(examples):
    for split in SPLITS:
        for example in examples[split]:
            for evidence_list in example["evidences"]:
                for evidence in evidence_list:
                    check_evidences_sent_len(evidence, example["annotation_id"])
...
def parse_label(example):
    assert example["classification"] in {"SUPPORTS", "REFUTES"}
    return 1 if example["classification"] == "SUPPORTS" else 0


def get_unique_rationale_indices(example):
    return list(set(get_rationale_indices(example)))


def get_rationale_indices(example):
    return [ev["start_sentence"] for evl in example["evidences"] for ev in evl]


def get_query(example):
    return [token.lower() for token in word_tokenize(example["query"])]


def get_sentences(example, docs):
    assert len(example["docids"]) == 1
    return docs[example["docids"][0]]


def prepare_example(example, docs):
    return (get_sentences(example, docs), parse_label(example), get_unique_rationale_indices(example), get_query(example))
