...
def validate(generator, classifier, annotation_loader, device):
    ...
    micro_precision = torch.true_divide(num_true_pos , num_predicted_pos)
    micro_recall =torch.true_divide( num_true_pos , num_real_pos)
    micro_f1 =torch.true_divide( 2 * (micro_precision * micro_recall) , (micro_precision +
                                                       micro_recall))
    sparsity =torch.true_divide( num_predicted_pos , num_words)
...
def validate_rationales(model, annotation_loader, device,writer_epoch):
    ...
    if writer is not None:
        ...
        writer.add_scalar('rat_pre', precision, epoch)
        writer.add_scalar('rat_rec', recall, epoch)
        writer.add_scalar('rat_f1', f1_score, epoch)
    print("rationale dataset : recall:{:.4f} precision:{:.4f} f1-score:{:.4f} accuracy:{:.4f}".format(recall, precision,
                                                                                                      f1_score,
                                                                                                      accuracy))

def validate_teacher(teacher,dataloader,device,test=False):
    TP = 0
    TN = 0
    FN = 0
    FP = 0
    ...
    with torch.no_grad():
        for (batch_id, data) in enumerate(dataloader):
            if test:
                inputs,masks,labels,rationale=data
            else:
