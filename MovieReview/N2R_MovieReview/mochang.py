...
# create model name
RNP_N2R_model_name = "RNP_N2R_model_" + "_".join(
    [f"{k}_{v}" for k, v in sorted(numeric_params.items())]
) + ".pth"

print(RNP_N2R_model_name)

# =========================================
# Common filenames for ALL generated files
# =========================================

base_model_name = os.path.splitext(RNP_N2R_model_name)[0]

save_dir = f"./trained_model/{args.data_type}"
os.makedirs(save_dir, exist_ok=True)

model_path = os.path.join(save_dir, f"{base_model_name}.pth")
results_path = os.path.join(save_dir, f"{base_model_name}.txt")
csv_path = os.path.join(save_dir, f"{base_model_name}.csv")
log_path = os.path.join(save_dir, f"{base_model_name}.log")
