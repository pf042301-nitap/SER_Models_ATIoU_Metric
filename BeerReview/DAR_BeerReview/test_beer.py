...
def parse():
    ...
    parser.add_argument('--batch_size',
                        type=int,
                        default=32,
                        help='Testing batch size [default: 32]')
    parser.add_argument('--train_batch_size',
                        type=int,
                        default=128,
                        help='Training batch size used for model selection')
    # pretrained embeddings
    parser.add_argument('--embedding_dir',
                        type=str,
                        default='./data/hotel/embeddings',
                        help='Dir. of pretrained embeddings')
    parser.add_argument('--embedding_name',
                        type=str,
                        default='glove.6B.100d.txt',
                        help='File name of pretrained embeddings')
    ...
    parser.add_argument('--sparsity_lambda',
                        type=float,
                        default=11.0,
                        help='Sparsity trade-off [default: 11.0]')
    parser.add_argument('--continuity_lambda',
                        type=float,
                        default=12.0,
                        help='Continuity trade-off [default: 12.0]')
    parser.add_argument('--sparsity_percentage',
                        type=float,
                        default=0.1,
                        help='Regularizer to control highlight percentage')
    parser.add_argument('--cls_lambda',
                        type=float,
                        default=0.9,
                        help='lambda for classification loss')
    parser.add_argument('--tea_lambda',
                        type=float,
                        default=0.9,
                        help='lambda for teacher loss')
    ...
    parser.add_argument('--model_base_dir',
                        type=str,
                        default='/home/dinesh/Documents/SXAI/DAR_Final/beer_results/trained_model/beer',
...
# Construct model path with .pkl extension matching DAR model pattern
# Pattern: DAR_model_share_{share}_sparsity_{sparsity_percentage}_lr_{lr}_conl_{continuity_lambda}_spl_{sparsity_lambda}_aspect_{aspect}.pkl

model_filename = f"DAR_model_share_{args.share}_sparsity_{args.sparsity_percentage}_lr_{args.lr}_conl_{args.continuity_lambda}_spl_{args.sparsity_lambda}_aspect_{args.aspect}.pkl"

# Try multiple possible paths
possible_paths = [
    os.path.join(args.model_base_dir, model_filename),
    os.path.join('./beer_results/trained_model/beer', model_filename),
...
with torch.no_grad():
    for batch_idx, (inputs, masks, labels, annotations) in enumerate(annotation_loader):
        ...
        # Forward pass - try different forward pass methods
        try:
            rationales, logits = model(inputs, masks)
            has_rationales = True
        except Exception as e:
            print(f"Error with forward pass: {e}")
        ...
        for i in range(batch_size):
            ...
            if has_rationales and rationales is not None:
                ...
                gen_rationale_mask = np.zeros_like(annotations[i].cpu().numpy())
...
# Calculate and print metrics on dev dataset
print("\nCalculating metrics on dev dataset...")
with torch.no_grad():
    TP = 0
    TN = 0
    FN = 0
...
# Calculate metrics on annotation dataset
print("\nCalculating metrics on annotation dataset...")
with torch.no_grad():
    TP = 0
    TN = 0
    FN = 0
...
print("\nCalculating sparsity metrics on annotation dataset...")
annotation_results = validate_share(model, annotation_loader, device)
print("Annotation dataset sparsity metrics:")
...
with open(results_filepath, 'w') as f:
    ...
    f.write(f"Model: DAR (Share={args.share})\n")
    ...
    f.write(f"tea_lambda: {args.tea_lambda}\n")
    f.write(f"epochs: {args.epochs}\n")
    f.write(f"pre_epoch: {args.pre_epoch}\n")
    f.write(f"max_length: {args.max_length}\n")
    f.write(f"seed: {args.seed}\n")
    f.write(f"train_batch_size: {args.train_batch_size}\n")
    f.write(f"\n--- Dev Dataset Results ---\n")
