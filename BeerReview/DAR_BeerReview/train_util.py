...
def train_teacher(teacher, optimizer, dataset, device):
    ...
    for (batch_id, (inputs, masks, labels)) in enumerate(dataset):
...
def train_share(model, optimizer, dataset, device, args,writer_epoch,teacher_model):
    TP = 0
    TN = 0
    FN = 0
    FP = 0
    ...
    for (batch, (inputs, masks, labels)) in enumerate(dataset):
        ...
        tea_logits=teacher_model.train_rationale(inputs,rationales,masks)

        # computer loss
        cls_loss = args.cls_lambda * F.cross_entropy(logits, labels)
        teacher_loss=args.tea_lambda * F.cross_entropy(tea_logits, labels)
        ...
        loss = cls_loss + sparsity_loss + continuity_loss+teacher_loss
